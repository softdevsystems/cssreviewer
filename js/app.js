(() => {
  ...
  function renderDashboard(){
    setPage('Dashboard','Review lessons, activities, flashcards, and practice tests for CSS NC II.');
    ...
    $('#view-dashboard').innerHTML=`
      <section class="hero">
        <div><span class="eyebrow">TESDA CSS NC II Reviewer</span><h2>Continue learning, ${esc((state.name||'Learner').split(' ')[0])}.</h2><p>Study the five CSS NC II modules through organized lessons, interactive activities, flashcards, and practice assessments.</p><div class="hero-actions"><button class="btn primary" data-open-lesson="${lesson.id}">Continue: ${esc(lesson.title)}</button><button class="btn" data-view-go="modules">Browse all lessons</button><button class="btn" data-view-go="about">About the creators</button></div></div>
        <div class="hero-art"><div class="hero-ring"><b>${D.modules.length}</b><span>modules</span></div><div class="hero-ring"><b>${D.lessons.length}</b><span>lessons</span></div><div class="hero-ring"><b>${D.lessons.reduce((n,l)=>n+l.pages,0)}</b><span>lesson pages</span></div></div>
      </section>
      ...
      <section class="next-grid">
        <article class="next-card"><span class="next-icon">▤</span><div><b>Review lesson pages</b><p>Read the lesson content page by page and use the zoom controls when needed.</p></div><button class="btn small" data-open-lesson="${lesson.id}" data-tab="pages">Open lesson</button></article>
        <article class="next-card"><span class="next-icon">◫</span><div><b>Complete interactive activities</b><p>Practice matching, classification, sequencing, self-checks, forms, and checklists.</p></div><button class="btn small" data-view-go="activities">Open activities</button></article>
  ...
  function renderModules(){
    setPage('Modules & Lessons','Study the five CSS NC II modules and their lessons.');
  ...
  function renderLesson(){
    ...
    const m=moduleById(l.module); setPage(l.title,`${m.code} • ${l.group} • ${l.pages} lesson pages`);
    ...
    $('#view-lesson').innerHTML=`
      ...
      <section class="lesson-hero"><div><span class="eyebrow">${m.code} · ${esc(l.group)} · ${esc(l.number)}</span><h2>${esc(l.title)}</h2><p>${esc(l.summary)}</p><div class="lesson-actions"><button class="btn primary" id="completeLessonBtn">${done?'✓ Lesson completed':'Mark lesson complete'}</button><a class="btn" href="${l.pdf}" target="_blank" rel="noopener">Open PDF</a><button class="btn" id="printLessonBtn">Print lesson</button></div></div><img src="${l.pagesData[Math.min(2,l.pagesData.length-1)].image}" alt="${esc(l.title)} lesson visual"></section>
      <div class="tabs" role="tablist"><button class="tab ${currentLessonTab==='overview'?'active':''}" data-lesson-tab="overview">Overview</button><button class="tab ${currentLessonTab==='pages'?'active':''}" data-lesson-tab="pages">Lesson Pages (${l.pages})</button><button class="tab ${currentLessonTab==='activities'?'active':''}" data-lesson-tab="activities">Activities (${l.activityIds.length})</button><button class="tab ${currentLessonTab==='checklist'?'active':''}" data-lesson-tab="checklist">Study Checklist</button></div>
  ...
  function renderLessonTab(){
    ...
    if(currentLessonTab==='overview'){
      box.innerHTML=`<section class="content-grid"><article class="panel"><span class="eyebrow">Lesson scope</span><h3>Topics included</h3><ol class="topic-list">${l.topics.map(t=>`<li>${esc(t)}</li>`).join('')}</ol><div class="source-note"><b>Lesson guide</b><p>Review the topics, lesson pages, activities, and study checklist to complete this lesson.</p></div></article><article class="panel"><span class="eyebrow">Quick access</span><h3>Lesson resources</h3><div class="resource-list"><button data-lesson-tab-go="pages"><b>Read all ${l.pages} lesson pages</b><span>Page viewer with zoom and text</span></button><button data-lesson-tab-go="activities"><b>Complete ${l.activityIds.length} activities</b><span>Interactive and practical exercises</span></button><a href="${l.pdf}" target="_blank"><b>Open PDF</b><span>View or save the lesson PDF</span></a></div></article></section><section class="panel"><div class="section-head compact"><div><span class="eyebrow">Activities in this lesson</span><h3>Interactive practice</h3></div></div><div class="activity-grid">${l.activityIds.map(id=>activityCard(activityById(id))).join('')}</div></section>`;
  ...
  function renderPageViewer(){
    ...
    $('#lessonTabContent').innerHTML=`<section class="page-viewer"><div class="page-toolbar"><div><b>Page ${currentPage} of ${l.pages}</b><span>${esc(l.title)}</span></div><div class="page-nav"><button class="btn small" id="prevPageBtn" ${currentPage===1?'disabled':''}>← Previous</button><select id="pageSelect" aria-label="Select page">${l.pagesData.map(x=>`<option value="${x.n}" ${x.n===currentPage?'selected':''}>Page ${x.n}</option>`).join('')}</select><button class="btn small" id="nextPageBtn" ${currentPage===l.pages?'disabled':''}>Next →</button></div></div><div class="source-page"><button class="page-image-button" id="expandPageBtn" aria-label="Expand lesson page"><img src="${p.image}" alt="${esc(l.title)}, page ${currentPage}" loading="eager"></button><details class="page-text"><summary>Show lesson text</summary><pre>${esc(p.text||'Text is not available for this page. Refer to the lesson image above.')}</pre></details></div><div class="thumbnail-strip">${l.pagesData.map(x=>`<button class="thumb ${x.n===currentPage?'active':''}" data-page="${x.n}" title="Page ${x.n}"><img src="${x.image}" alt="Page ${x.n}" loading="lazy"><span>${x.n}</span></button>`).join('')}</div></section>`;
  }
  ...
  function renderActivities(){
    setPage('Interactive Activities','Practice the lessons through interactive exercises and practical activities.');
  ...
  function renderActivity(a){
    const lesson=lessonById(a.lessonId);setPage(a.title,`${lesson.title} • ${activityTypeLabel(a.type)}`);$('#view-activity').innerHTML=`<button class="back-link" data-view-go="activities">← Back to activities</button><section class="activity-work"><div class="activity-work-head"><div class="activity-icon large">${activityIcon(a.type)}</div><div><span class="eyebrow">${moduleById(lesson.module).code} · ${esc(lesson.title)}</span><h2>${esc(a.title)}</h2><p>${esc(a.instruction)}</p></div></div><div id="activityBody"></div><section id="activityContinue" class="feedback success" aria-live="polite" hidden></section></section>`;
  ...
  function feedback(score,total,msg=''){return `<div class="feedback ${score===total?'success':'review'}"><b>${score}/${total} correct</b><p>${esc(msg|| (score===total?'Excellent. You completed the activity correctly.':'Review the highlighted answers and the lesson.'))}</p></div>`;}
  ...
  function renderTopology(a,body){
    ...
    $('#saveTopologyBtn',body).onclick=()=>{save();const pcs=nodes.filter(n=>n.type==='pc').length;$('#activityFeedback',body).innerHTML=`<div class="feedback ${pcs>=10?'success':'review'}"><b>${nodes.length} nodes and ${links.length} links saved</b><p>${pcs>=10?'The design includes at least ten worker PCs.':'The activity requires at least ten worker PCs. Add more PCs if each worker needs a workstation.'}</p></div>`;toast('Topology saved');};
  ...
  function renderFlashcards(){
    setPage('Flashcards','Review key terms from all five modules.');
  ...
  function renderAssessment(){
    setPage('Practice Test',`Create a randomized practice test from ${D.questions.length} questions.`);
  ...
  function renderSources(){setPage('Learning Materials','Open lesson PDFs or review each lesson page in the website.');const totalPages=D.lessons.reduce((n,l)=>n+l.pages,0);$('#view-sources').innerHTML=`<section class="source-intro"><div><span class="eyebrow">CSS NC II learning materials</span><h2>Lessons and reference pages</h2><p>Open each lesson in the reviewer or view its PDF for focused study and review.</p></div><div class="source-stat"><b>${totalPages}</b><span>lesson pages</span></div></section>${D.modules.map(m=>`<section class="source-module"><div class="group-title"><div><span class="eyebrow">${m.code}</span><h3>${esc(m.title)}</h3></div><span>${m.lessonIds.reduce((n,id)=>n+lessonById(id).pages,0)} pages</span></div><div class="source-list">${m.lessonIds.map(id=>{const l=lessonById(id);return `<article><img src="${l.pagesData[0].image}" alt="Preview of ${esc(l.title)}" loading="lazy"><div><span>${esc(l.number)} · ${l.pages} pages</span><h4>${esc(l.title)}</h4><p>${esc(l.course)}</p><div><button class="btn small" data-open-lesson="${l.id}" data-tab="pages">Read lesson</button><a class="btn small" href="${l.pdf}" target="_blank">Open PDF</a></div></div></article>`;}).join('')}</div></section>`).join('')}`;}
