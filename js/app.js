(() => {
  ...
  function renderClassify(a,body){
    body.innerHTML=`<div class="classify-list">${a.items.map((it,i)=>`<article class="classify-item"><b>${esc(it.text)}</b><div>${a.categories.map(c=>`<button data-classify-item="${i}" data-category="${esc(c)}">${esc(c)}</button>`).join('')}</div></article>`).join('')}</div><div class="activity-actions"><button class="btn primary" id="checkActivityBtn">Check answers</button></div><div id="activityFeedback"></div>`;
    const answers={};$$('[data-classify-item]',body).forEach(b=>b.onclick=()=>{const i=Number(b.dataset.classifyItem);answers[i]=b.dataset.category;$$(`[data-classify-item="${i}"]`,body).forEach(x=>x.classList.toggle('selected',x===b));});
    $('#checkActivityBtn').onclick=()=>{let score=0;a.items.forEach((it,i)=>{const ok=answers[i]===it.category;if(ok)score++;$$(`[data-classify-item="${i}"]`,body).forEach(x=>x.classList.toggle('correct',ok&&x.dataset.category===answers[i]));});$('#activityFeedback').innerHTML=feedback(score,a.items.length);completeActivity(a,score,a.items.length);};
  }
  ...
  function renderAbout(){
    setPage('About','Profile page for the website creators and SoftDev Systems.');
    $('#view-about').innerHTML=`
      <section class="about-page">
        <section class="about-hero-card panel">
          <div class="about-hero-copy">
            <span class="eyebrow">Created and Developed by</span>
            <h2>Reyan Bracino</h2>
            <p class="about-lead">System Analyst and System Developer</p>
            <p>This TESDA CSS NC II reviewer website was created and developed by Reyan Bracino and is powered by SoftDev Systems. It is designed as a responsive offline-capable reviewer for learners using phones, tablets, laptops, and desktop devices.</p>
            <div class="about-chip-row">
              <span class="about-chip">System Analyst</span>
              <span class="about-chip">System Developer</span>
              <span class="about-chip">Responsive PWA</span>
              <span class="about-chip">Offline Ready</span>
            </div>
          </div>
          <div class="about-logo-card">
            ...
            <div class="about-logo-meta">
              ...
              <span>Software solutions • systems development • digital tools</span>
            </div>
          </div>
        </section>

        <section class="about-grid">
          <article class="panel about-profile-card">
            <span class="eyebrow">Developer profile</span>
            <div class="about-profile-head">
              <div class="about-avatar">RB</div>
              <div>
                <h3>Reyan Bracino</h3>
                <p>System Analyst and System Developer</p>
              </div>
            </div>
            <ul class="about-list">
              <li>Created and developed the reviewer website layout, navigation, offline support, and activity flow.</li>
              <li>Designed the learning experience for responsive study across mobile and desktop devices.</li>
              <li>Powered the project using SoftDev Systems.</li>
            </ul>
          </article>
          ...
          <article class="panel about-profile-card">
            ...
            <div class="about-profile-head">
              <div class="about-avatar secondary">AG</div>
              <div>
                <h3>Advent Grace Montesa-Bracino</h3>
                <p>Trainer</p>
              </div>
            </div>
            <ul class="about-list">
              <li>Trainer support and learning guidance for the reviewer project.</li>
              <li>Helps align the reviewer content for learner-friendly presentation.</li>
            </ul>
          </article>

          <article class="panel about-contact-card">
            <span class="eyebrow">Contact details</span>
            <h3>Get in touch</h3>
            <div class="about-contact-list">
              <a class="about-contact-item" href="tel:09755274378"><span>☎</span><div><b>Contact Number</b><small>09755274378</small></div></a>
              <a class="about-contact-item" href="https://www.facebook.com/softdevph" target="_blank" rel="noopener"><span>f</span><div><b>Facebook Page</b><small>facebook.com/softdevph</small></div></a>
            </div>
            <div class="about-action-row">
              <a class="btn primary" href="tel:09755274378">Call now</a>
              <a class="btn" href="https://www.facebook.com/softdevph" target="_blank" rel="noopener">Open Facebook page</a>
            </div>
          </article>

          <article class="panel about-softdev-card">
            <span class="eyebrow">SoftDev Systems</span>
            <h3>Digital solutions for learning and systems development</h3>
            <p>SoftDev Systems powers this project with a focus on useful, responsive, and practical software solutions.</p>
            <div class="about-mini-brand">
              <img src="./assets/branding/softdev-logo.jpg" alt="SoftDev Systems logo">
              <div>
                <b>SoftDev Systems</b>
                <small>Powered by SoftDev Systems</small>
              </div>
            </div>
          </article>
        </section>
  ...
  $('#menuBtn').onclick=()=>{$('#sidebar').classList.add('open');$('#sidebarBackdrop').classList.add('show');};$('#sidebarClose').onclick=closeSidebar;$('#sidebarBackdrop').onclick=closeSidebar;
