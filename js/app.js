(() => {
  ...
  function parseRoute(){
    ...
    const valid=['dashboard','modules','activities','flashcards','assessment','glossary','sources','settings'];
  ...
  function navigate(view,opts={}){
    ...
    const renders={dashboard:renderDashboard,modules:renderModules,lesson:renderLesson,activities:renderActivities,activity:()=>{const a=activityById(activeActivityId);if(a)renderActivity(a);},flashcards:renderFlashcards,assessment:renderAssessment,quiz:renderQuiz,results:renderResults,glossary:renderGlossary,sources:renderSources,settings:renderSettings};
  ...
  function completeActivity(a,score,total,extra={}){state.activityResults[a.id]={completed:true,score:total?`${score}/${total}`:'Completed',numeric:total?pct(score,total):100,updated:new Date().toISOString(),...extra};saveState();toast(total?`Activity score: ${score}/${total}`:'Activity saved');}
  ...
  function renderRisk(a,body){
    ...
    $('#riskForm').onsubmit=e=>{e.preventDefault();const data=Object.fromEntries(new FormData(e.target));state.activityResults[a.id]={completed:true,score:'Completed',numeric:100,data,updated:new Date().toISOString()};saveState();toast('Risk assessment saved');$('#activityFeedback').innerHTML='<div class="feedback success"><b>Assessment saved</b><p>Your worksheet is stored in this browser.</p></div>';};$('#printWorksheetBtn').onclick=()=>window.print();
  }
  ...
  function renderTopology(a,body){
    ...
    const save=()=>{const data={nodes,links};state.activityResults[a.id]={completed:true,score:`${nodes.length} nodes`,numeric:Math.min(100,Math.round(nodes.length/10*100)),data,updated:new Date().toISOString()};saveState();return data;};
